  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex rounded-lg bg-gray-900 p-1 border border-gray-800">
        <button onClick={() => setActiveTab('active')} className={`flex-1 text-sm font-semibold py-2 min-touch-target rounded-md transition-all-200 ${activeTab === 'active' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}>{t('lists.active')}</button>
        <button onClick={() => setActiveTab('archived')} className={`flex-1 text-sm font-semibold py-2 min-touch-target rounded-md transition-all-200 ${activeTab === 'archived' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}>{t('lists.archived')}</button>
      </div>
      ...
      {displayedLists.length === 0 ? (
        <div className="glass-panel rounded-2xl p-8 text-center border border-gray-800/60">
          ...
          <p className="text-gray-400 text-sm font-medium">{t('lists.noLists').replace('{tab}', activeTab === 'active' ? t('lists.active').toLowerCase() : t('lists.archived').toLowerCase())}</p>
        </div>
      ) : (
            ...
            <div key={list.id} onClick={() => onSelectList(list.id)} className="glass-panel-interactive rounded-2xl p-4 border border-gray-800/60 shadow-sm cursor-pointer flex justify-between items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`px-2 py-0.5 rounded-md text-[9px] font-bold tracking-wider ${list.status === 'ordered' ? 'bg-teal-500/10 text-teal-400 border border-teal-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                    {list.status.toUpperCase() === 'ACTIVE' ? t('lists.statusActive') : list.status.toUpperCase() === 'ORDERED' ? t('lists.statusOrdered') : t('lists.statusDraft')}
                  </span>
