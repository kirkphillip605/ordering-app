  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex rounded-lg bg-gray-900 p-1 border border-gray-800">
        <button onClick={() => setActiveTab('active')} className={`flex-1 text-sm font-semibold py-2 rounded-md transition-all-200 ${activeTab === 'active' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}>Active</button>
        <button onClick={() => setActiveTab('archived')} className={`flex-1 text-sm font-semibold py-2 rounded-md transition-all-200 ${activeTab === 'archived' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'}`}>Archived</button>
      </div>
      ...
      {displayedLists.length === 0 ? (
        <div className="glass-panel rounded-2xl p-8 text-center border border-gray-800/60">
          ...
          <p className="text-gray-400 text-sm font-medium">No {activeTab} lists</p>
        </div>
      ) : (
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-[10px] text-gray-600 font-medium ml-1">
                    {list.orderDate ? `Ordered: ${formatDate(list.orderDate)}` : `Created: ${formatDate(list.createdAt)}`}
                  </span>
                </div>
              </div>
