...
export function VendorsTab({ token, vendors, fetchVendors }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      showToast(form.id ? 'Vendor updated' : 'Vendor created');
      fetchVendors();
      setShowModal(false);
    } else showToast('Failed to save vendor', 'error');
  };

  const handleDelete = (id: string) => {
    setConfirmState({
      isOpen: true, title: 'Delete Vendor', message: 'Are you sure?', action: async () => {
        const res = await fetch(`/api/vendors/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
        if (res.ok) { showToast('Vendor deleted'); fetchVendors(); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setForm({ id: '', name: '', description: '' }); setShowModal(true); }} className="px-4 py-2 rounded-xl bg-teal-500 hover:bg-teal-600 text-gray-950 text-xs font-bold shadow-lg transition-all-200 flex items-center gap-1">
          <IonIcon icon={addOutline} /> Add Vendor
        </button>
      </div>
      ...
      {vendors.length === 0 ? (
        <div className="glass-panel rounded-2xl p-8 text-center border border-gray-800/60">
          ...
          <p className="text-gray-400 text-sm font-medium">No vendors registered</p>
        </div>
      ) : (
          ...
          {vendors.map(vendor => (
            <div key={vendor.id} className="glass-panel rounded-2xl p-4 border border-gray-800/60 shadow-sm flex justify-between items-center">
              ...
              <div className="flex gap-1">
                <button onClick={() => { setForm({ ...vendor, description: vendor.description||'' }); setShowModal(true); }} className="p-2 rounded-xl hover:bg-gray-800 text-gray-400 hover:text-white transition-all-200"><IonIcon icon={createOutline} /></button>
                <button onClick={() => handleDelete(vendor.id)} className="p-2 rounded-xl hover:bg-rose-500/10 text-rose-400 transition-all-200"><IonIcon icon={trashOutline} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <IonModal isOpen={showModal} onDidDismiss={() => setShowModal(false)}>
        <div className="p-6 bg-gray-900 text-gray-100 rounded-t-3xl border-t border-gray-800">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-white">{form.id ? 'Edit Vendor' : 'Add Vendor'}</h2>
          ...
          <form onSubmit={handleSave} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-gray-950 border border-gray-800 text-white focus:border-teal-500/50 focus:outline-none transition-all-200" />
            </div>
